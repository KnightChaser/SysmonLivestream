...
function Invoke-SysmonServiceOperation {
    ...
    if ((Test-Path -Path $Sysmon64RegistryPath) -and (Test-Path -Path $Sysmon64DriverPath)){
        Write-Host "[OK] Sysmon is already installed on the system(registry key and driver file exist)."
        exit 0
    } else {
        Write-Host "[..] Sysmon is not installed on the system. Proceeding to setting up Sysmon."
    }
}
